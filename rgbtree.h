...
public:
  ...
  /* nearest neighbor function.                                     */
    ...
    */
    ...
    pixel r,g,b value
    */
   int getDim(const RGBAPixel& pixel, int d) const;


   /*
   * Helper function for findNearestNeighbor
   */ 
   RGBAPixel workDown(const RGBAPixel& query, int start, int end, int currMinDist, RGBAPixel nn, int level) const;

   /*
   * Helper to compute Euclidean dist squared between two pixels
   */
   int computeDist(const RGBAPixel& first, const RGBAPixel& second) const;

   void build(int start, int end, int median, int level);
